...

                     OpenID Connect Registration 1.0       December 2023
...
Table of Contents
   ...
   8.  Implementation Considerations  . . . . . . . . . . . . . . . . 25
     8.1.  Compatibility Notes  . . . . . . . . . . . . . . . . . . . 25
     8.2.  Implementation Notes on Stateless Dynamic Client
           Registration . . . . . . . . . . . . . . . . . . . . . . . 25
   9.  Security Considerations  . . . . . . . . . . . . . . . . . . . 26
     9.1.  Impersonation  . . . . . . . . . . . . . . . . . . . . . . 26
     9.2.  Native Code Leakage  . . . . . . . . . . . . . . . . . . . 26
     9.3.  TLS Requirements . . . . . . . . . . . . . . . . . . . . . 26
   10. IANA Considerations  . . . . . . . . . . . . . . . . . . . . . 28
     10.1. OAuth Dynamic Client Registration Metadata Registration  . 28
       10.1.1.  Registry Contents . . . . . . . . . . . . . . . . . . 28
     10.2. OAuth Token Endpoint Authentication Methods
           Registration . . . . . . . . . . . . . . . . . . . . . . . 31
       10.2.1.  Registry Contents . . . . . . . . . . . . . . . . . . 31
   11. References . . . . . . . . . . . . . . . . . . . . . . . . . . 32
...
1.  Introduction
   ...
   ID.  This specification describes how an RP can register with an OP,
   and how registration information for the RP can be retrieved.

   The previous versions of this specification are:

   o  OpenID Connect Registration 1.0 incorporating errata set 1
      [OpenID.Registration.Errata1]

   o  OpenID Connect Registration 1.0 (final)
...

   ...
   the terms "JSON Web Token (JWT)" and "Nested JWT" defined by JSON Web
   Token (JWT) [JWT], the term "Base64url Encoding" defined by JSON Web
   Signature (JWS) [JWS], and the terms defined by OpenID Connect Core
   1.0 [OpenID.Core].
...
2.  Client Metadata
   ...
   response_types
      OPTIONAL.  JSON [RFC8259] array containing a list of the OAuth 2.0
...

   ...
   application_type
      ...
      "redirect_uris" using custom URI schemes or loopback URLs using
      the "http" scheme; loopback URLs use "localhost" or the IP
      ...
      using the "http" scheme, other than the loopback case for Native
...

   ...
   policy_uri
      ...
      User to read about how the profile data will be used.  The value
      of this field MUST point to a valid web page.  The OpenID Provider
   ...
   jwks_uri
      OPTIONAL.  URL for the Client's JWK Set [JWK] document, which MUST
      use the "https" scheme.  If the Client signs requests to the
      Server, it contains the signing key(s) the Server uses to validate
      signatures from the Client.  The JWK Set MAY also contain the
      Client's encryption keys(s), which are used by the Server to
      encrypt responses to the Client.  When both signing and encryption
      keys are made available, a "use" (public key use) parameter value
      is REQUIRED for all keys in the referenced JWK Set to indicate
      each key's intended usage.  Although some algorithms allow the
      same key to be used for both signatures and encryption, doing so
      is NOT RECOMMENDED, as it is less secure.  The JWK "x5c" parameter
      MAY be used to provide X.509 representations of keys provided.
      When used, the bare key values MUST still be present and MUST
      match those in the certificate.  The JWK Set MUST NOT contain
      private or symmetric key values.

   jwks
      OPTIONAL.  Client's JWK Set [JWK] document, passed by value.  The
      semantics of the "jwks" parameter are the same as the "jwks_uri"
      parameter, other than that the JWK Set is passed by value, rather
      than by reference.  This parameter is intended only to be used by
      Clients that, for some reason, are unable to use the "jwks_uri"
...

      ...
      can use "jwks_uri", it MUST NOT use "jwks".  One significant
      downside of "jwks" is that it does not enable key rotation (which
      "jwks_uri" does, as described in Section 10 of OpenID Connect Core
      1.0 [OpenID.Core]).  The "jwks_uri" and "jwks" parameters MUST NOT
      be used together.  The JWK Set MUST NOT contain private or
   ...
   subject_type
      ...
      of the supported "subject_type" values for the OP.  Valid types
   ...
   id_token_encrypted_response_enc
      ...
      "id_token_encrypted_response_alg" is specified, the default
...

   ...
   userinfo_signed_response_alg
      ...
      the UserInfo Response to return the Claims as a UTF-8 [RFC3629]
   ...
   userinfo_encrypted_response_enc
      ...
      specified, the default "userinfo_encrypted_response_enc" value is
      "A128CBC-HS256".  When "userinfo_encrypted_response_enc" is
...

   ...
   request_object_encryption_enc
      ...
      "request_object_encryption_alg" is specified, the default
...

   ...
   default_acr_values
      ...
      the OP.  Values specified in the "acr_values" request parameter or
   ...
   request_uris
      ...
      by the RP for use at the OP.  These URLs MUST use the "https"
      scheme unless the target Request Object is signed in a way that is
      verifiable by the OP.  Servers MAY cache the contents of the files
      ...
      used be pre-registered with the "require_request_uri_registration"
      discovery parameter.
...
3.  Client Registration Endpoint
   ...
   developers.  The Client Registration Endpoint SHOULD support the use
   of Cross-Origin Resource Sharing (CORS) [CORS] and/or other methods
   as appropriate to enable JavaScript Clients and other Browser-Based
...
3.2.  Client Registration Response
   ...
   requested field values, other than the "redirect_uris" value, and
   substitute them with suitable values.  If this happens, the
   Authorization Server MUST include these fields in the response to the
   Client.  An Authorization Server MAY ignore values provided by the
   client and MUST ignore any fields sent by the Client that it does not
   understand.
...

   ...
   return a JSON document [RFC8259] using the "application/json" content
   ...
   registration_client_uri
      ...
      operations upon the resulting Client registration.  This URL MUST
      use the "https" scheme.  Implementations MUST either return both a
      Client Configuration Endpoint and a Registration Access Token or
      neither of them.
...
4.  Client Configuration Endpoint
   ...
   OAuth 2.0 Bearer Token [RFC6750].  The Client Configuration Endpoint
   SHOULD support the use of Cross-Origin Resource Sharing (CORS) [CORS]
   and/or other methods as appropriate to enable JavaScript Clients and
   other Browser-Based Clients to access it.

   Operations on this endpoint are switched through the use of different
   HTTP methods [RFC7231].  The only method defined for use at this
...
6.  String Operations
   ...
   names such as "client_id".  Comparing Unicode [UNICODE] strings,
...
8.  Implementation Considerations
   ...
   This specification is compatible with the OAuth 2.0 Dynamic Client
   Registration Protocol [RFC7591].  Implementations MAY use additional
   Client Metadata values defined by [RFC7591], such as
   "software_statement", should they facilitate their use cases.
   ...
   such as Update, can do so using that specification, while being
   mindful that the specification is experimental.
...
9.1.  Impersonation
   ...
   phishing risk, since the logo could confuse users into thinking they
   are logging in to the legitimate RP.  An OP could also warn if the
   domain/site of the logo does not match the domain/site of registered
   Redirection URIs.  An OP can also make warnings against untrusted RPs
   in all cases, especially if they are dynamically registered, have not
   been trusted by any users at the OP before, and want to use the logo
   feature.
...
10.1.1.  Registry Contents
   ...
   o  Client Metadata Name: "application_type"
   o  Client Metadata Description: Kind of the application -- "native"
      or "web"
   ...
   o  Client Metadata Name: "sector_identifier_uri"
   o  Client Metadata Description: URL using the "https" scheme to be
      used in calculating Pseudonymous Identifiers by the OP
   o  Change Controller: OpenID Foundation Artifact Binding Working
      Group - openid-specs-ab@lists.openid.net
   o  Specification Document(s): Section 2 of this specification

   o  Client Metadata Name: "subject_type"
   o  Client Metadata Description: "subject_type" requested for
      responses to this Client -- "pairwise" or "public"
   o  Change Controller: OpenID Foundation Artifact Binding Working
      Group - openid-specs-ab@lists.openid.net
   o  Specification Document(s): Section 2 of this specification

   o  Client Metadata Name: "id_token_signed_response_alg"
   o  Client Metadata Description: JWS "alg" algorithm REQUIRED for
      signing the ID Token issued to this Client
   o  Change Controller: OpenID Foundation Artifact Binding Working
      Group - openid-specs-ab@lists.openid.net
   o  Specification Document(s): Section 2 of this specification

   o  Client Metadata Name: "id_token_encrypted_response_alg"
   o  Client Metadata Description: JWE "alg" algorithm REQUIRED for
      encrypting the ID Token issued to this Client
   o  Change Controller: OpenID Foundation Artifact Binding Working
      Group - openid-specs-ab@lists.openid.net
   o  Specification Document(s): Section 2 of this specification

   o  Client Metadata Name: "id_token_encrypted_response_enc"
...

   ...
   o  Client Metadata Description: JWE "enc" algorithm REQUIRED for
      encrypting the ID Token issued to this Client
   o  Change Controller: OpenID Foundation Artifact Binding Working
      Group - openid-specs-ab@lists.openid.net
   o  Specification Document(s): Section 2 of this specification

   o  Client Metadata Name: "userinfo_signed_response_alg"
   o  Client Metadata Description: JWS "alg" algorithm REQUIRED for
      signing UserInfo Responses
   o  Change Controller: OpenID Foundation Artifact Binding Working
      Group - openid-specs-ab@lists.openid.net
   o  Specification Document(s): Section 2 of this specification

   o  Client Metadata Name: "userinfo_encrypted_response_alg"
   o  Client Metadata Description: JWE "alg" algorithm REQUIRED for
      encrypting UserInfo Responses
   o  Change Controller: OpenID Foundation Artifact Binding Working
      Group - openid-specs-ab@lists.openid.net
   o  Specification Document(s): Section 2 of this specification

   o  Client Metadata Name: "userinfo_encrypted_response_enc"
   o  Client Metadata Description: JWE "enc" algorithm REQUIRED for
      encrypting UserInfo Responses
   o  Change Controller: OpenID Foundation Artifact Binding Working
      Group - openid-specs-ab@lists.openid.net
   ...
   o  Client Metadata Name: "request_object_signing_alg"
   o  Client Metadata Description: JWS "alg" algorithm that MUST be used
      for signing Request Objects sent to the OP
   o  Change Controller: OpenID Foundation Artifact Binding Working
      Group - openid-specs-ab@lists.openid.net
   o  Specification Document(s): Section 2 of this specification

   o  Client Metadata Name: "request_object_encryption_alg"
   o  Client Metadata Description: JWE "alg" algorithm the RP is
      declaring that it may use for encrypting Request Objects sent to
      the OP
   o  Change Controller: OpenID Foundation Artifact Binding Working
      Group - openid-specs-ab@lists.openid.net
   o  Specification Document(s): Section 2 of this specification

   o  Client Metadata Name: "request_object_encryption_enc"
   o  Client Metadata Description: JWE "enc" algorithm the RP is
      declaring that it may use for encrypting Request Objects sent to
      the OP
...

   ...
   o  Client Metadata Name: "token_endpoint_auth_signing_alg"
   o  Client Metadata Description: JWS "alg" algorithm that MUST be used
      for signing the JWT used to authenticate the Client at the Token
      Endpoint for the "private_key_jwt" and "client_secret_jwt"
   ...
   o  Client Metadata Name: "default_max_age"
   o  Client Metadata Description: Default Maximum Authentication Age
   o  Change Controller: OpenID Foundation Artifact Binding Working
      Group - openid-specs-ab@lists.openid.net
   o  Specification Document(s): Section 2 of this specification

   o  Client Metadata Name: "require_auth_time"
   o  Client Metadata Description: Boolean value specifying whether the
      "auth_time" Claim in the ID Token is REQUIRED
   o  Change Controller: OpenID Foundation Artifact Binding Working
      Group - openid-specs-ab@lists.openid.net
   o  Specification Document(s): Section 2 of this specification

   o  Client Metadata Name: "default_acr_values"
   o  Client Metadata Description: Default requested Authentication
      Context Class Reference values
   o  Change Controller: OpenID Foundation Artifact Binding Working
      Group - openid-specs-ab@lists.openid.net
   o  Specification Document(s): Section 2 of this specification

   o  Client Metadata Name: "initiate_login_uri"
   o  Client Metadata Description: URI using the "https" scheme that a
      third party can use to initiate a login by the RP
   o  Change Controller: OpenID Foundation Artifact Binding Working
      Group - openid-specs-ab@lists.openid.net
   o  Specification Document(s): Section 2 of this specification

   o  Client Metadata Name: "request_uris"
   o  Client Metadata Description: Array of "request_uri" values that
...
10.2.  OAuth Token Endpoint Authentication Methods Registration
   ...
   This specification registers the following token endpoint
   authentication methods in the IANA "OAuth Token Endpoint
   Authentication Methods" registry [IANA.OAuth.Parameters] established
   by [RFC7591]:

10.2.1.  Registry Contents

   o  Token Endpoint Authentication Method Name: "client_secret_jwt"
   o  Change Controller: OpenID Foundation Artifact Binding Working
      Group - openid-specs-ab@lists.openid.net
   o  Specification Document(s): Section 9 of OpenID Connect Core 1.0
      [OpenID.Core]

   o  Token Endpoint Authentication Method Name: "private_key_jwt"
   o  Change Controller: OpenID Foundation Artifact Binding Working
      Group - openid-specs-ab@lists.openid.net
   o  Specification Document(s): Section 9 of OpenID Connect Core 1.0
      [OpenID.Core]
...
11.2.  Informative References
   ...
   [OpenID.Registration.Errata1]
              ...
              1", November 2014, <https://openid.net/specs/
              openid-connect-registration-1_0-errata1.html>.

   [OpenID.Registration.Final]
              Sakimura, N., Bradley, J., and M. Jones, "OpenID Connect
              Dynamic Client Registration 1.0 (final)", February 2014, <
              https://openid.net/specs/
              openid-connect-registration-1_0-final.html>.
