...
```json
{
    ...
    "extends#integrity": "sha256-9a8b7c6d5e4f3a2b1c0...d4e5f6a7b8c9d0",
    "schema_uri": "https://pay.example.com/card-schema-1.0",
    "schema_uri#integrity": "sha256-1f2e3d4c5b6a7980812...3c4d5e6f7a8b9c",
    "transaction_data_types": [
        {
            ...
            "ui_labels_uri#integrity": "sha256-335f3750519114d2a93...5e6ed24c30c0317"
        }
    ]
}
```
...
To do so, this specification introduces and standardises a `payload` object and `subtype` string under [OID4VP]'s `transaction_data`.
...
Upon receiving a presentation request containing a `transaction_data` array, the Wallet Unit **SHALL** perform the following steps:

1. The Wallet Unit **SHALL** process each entry in the `transaction_data` array sequentially according to the [OpenId4VP] specification.
2. For each TS12 entry (i.e. contains a `payload` field), the Wallet Unit **SHALL** identify all credentials matching the presentation request that are SCA Attestations (indicated by the `category` claim with value `urn:eu:europa:ec:eudi:sua:sca` in their type metadata).
3. The Wallet Unit **SHALL** filter these credentials to include only those whose type metadata contains a `transaction_data_types` entry matching the `type` and `subtype` of the current `transaction_data` entry.
4. If no matching credentials are found for the current entry, the Wallet Unit **SHALL** cease processing and inform the User of the error.
5. For the currently selected credential, the Wallet Unit **SHALL** resolve the schema from the `type` field of the matched `transaction_data_types` entry. If the `type` is a URL, the Wallet Unit **SHALL** download it. If the `type` is not a URL, the Wallet Unit **SHALL** resolve it against the built-in transaction data types described in Section 4.3.
6. The Wallet Unit **SHALL** validate that the `transaction_data.payload` object conforms to the resolved [JSON Schema].
7. The Wallet Unit **SHALL** display the transaction data using the metadata (e.g., labels) defined in the selected credential's matched `transaction_data_types` entry.
8. If the User selects a different credential for the current entry, the Wallet Unit **SHALL** repeat the schema resolution, validation, and display steps using the new credential's metadata.
9. If validation fails for the selected credential, the Wallet Unit **SHALL** prevent the presentation of that credential.
10. If validation succeeds for all entries and the User consents, the Wallet Unit **MAY** proceed with the presentation.
...
- **Value**: An array of objects, where each object **SHALL** contain:
  - `locale`: A language identifier as defined in [RFC5646].
...
- **`transaction_data_types`**: **REQUIRED** (array) An array of objects, where each object contains:
  - **`type`**: **REQUIRED** (string) URI (URL or URN) that references a [JSON Schema] defining the structure of the `payload` object within the `transaction_data` object.
  - **`type#integrity`**: **CONDITIONAL** (string) Hash of the document referenced by `type`. **MUST** be present if `type` is a URL and integrity protection is desired.
  - **`subtype`**: **OPTIONAL** (string) A string that can be used to further categorize the transaction type. It is used to associate multiple different metadatas for transactions that have the same schema.
  ...
  - **`claims_uri#integrity`**: **CONDITIONAL** (string) Hash of the document referenced by `claims_uri`. **MUST** be present if `claims_uri` is present and integrity protection is desired.
  ...
  - **`ui_labels_uri#integrity`**: **CONDITIONAL** (string) Hash of the document referenced by `ui_labels_uri`. **MUST** be present if `ui_labels_uri` is present and integrity protection is desired.
...
Section 7 of [SD-JWT-VC] introduces a mechanism for Wallet Units to verify the integrity of external files referenced within Type Metadata. It is **RECOMMENDED** for SCA Attestation Providers to include this integrity mechanism in their SCA Attestation's type metadata.

If this mechanism is used, the SCA Attestation Provider **SHALL** include a hash value of the referenced file within the SCA Attestation's type metadata claim. The value **SHALL** conform to **[W3C.SRI]**, sections 3.1 and 3.2.1. This claim **SHALL** be suffixed with the `#integrity` designator (e.g., `schema_uri#integrity`) and **SHALL** be placed immediately adjacent to the claim it protects (i.e., as a sibling key).
...
The inclusion of the `#integrity` claims dictates that any modification to the SCA Attestation's type metadata or to any of the referenced files will cause a negative result of the integrity check, thereby invalidating the existing SCA Attestation. In consequence, re-issuance of the SCA attestation is necessary.

Non-normative example for an SD-JWT VC payload:
```json
{
    "iss": "https://issuer.superbank3.com",
    "sub": "37774a3f-ab14-43c3-96bc-bb1066a30a1d",
...
Non-normative example for an SD-JWT VC type metadata including integrity claims for the transaction data type array:
```json
{
  "vct": "https://pay.example.com/scheme",
  "name": "pay.example Payment Credential",
  "description": "This is a pay.example Payment Credential",
  "schema_uri": "https://pay.example.com/payment-scheme-schema-1.0",
  "schema_uri#integrity": "sha256-1f2e3d4c5b6a7980812...3c4d5e6f7a8b9c",
  "extends": "https://psd2.standard.org/payment_service_user/basic-schema-1.2",
  "extends#integrity": "sha256-9a8b7c6d5e4f3a2b1c0...d4e5f6a7b8c9d0",
  "category": "urn:eu:europa:ec:eudi:sua:sca",
  "transaction_data_types": [
    {
      "type": "https://pay.example.com/payment/v1.json",
      "type#integrity": "sha256-e3b0c44298fc49afbf4...a495991852b855",
      "claims_uri": "https://pay.example.com/payment/v1/claims.json",
      "claims_uri#integrity": "sha256-a1b2c3d4e5f67890123...456789abcdef012",
      "ui_labels_uri": "https://pay.example.com/payment/v1/labels.json",
      "ui_labels_uri#integrity": "sha256-335f3750519114d2a93...5e6ed24c30c0317"
    }
  ]
}
...
### 4.1.3 Merging of Transaction Data Types

When the `extends` mechanism of [SD-JWT-VC] is used, the `transaction_data_types` array in the extending metadata **SHALL** be merged with the `transaction_data_types` array of the extended (parent) metadata.

The merging process **SHALL** follow these rules:

1.  **Resolution**: Before any merging takes place, any `claims_uri` or `ui_labels_uri` present in any of the metadata files in the chain **SHALL** be resolved to their respective `claims` array or `ui_labels` object.
2.  **Inheritance Chain**: The merging process starts at the bottom of the inheritance chain and works its way up. For example, if metadata A extends metadata B, and metadata B extends metadata C, the Wallet Unit **SHALL** first merge B into C, and then merge A into the result of the previous merge.
3.  **Matching Entries**: Entries in the `transaction_data_types` arrays are matched based on the combination of `type` and `subtype`. If `subtype` is missing, it is treated as a null value.
4.  **Merging Entries**:
    *   If an entry in the extending metadata matches an entry in the parent metadata, the two entries are merged recursively.
    *   Entries in the extending metadata that do not match any entry in the parent metadata are appended to the result.
    *   Entries in the parent metadata that do not match any entry in the extending metadata are included in the result.

When merging two matching `transaction_data_types` entries:

*   **`claims` Array**:
    *   Entries are matched based on the `path` parameter.
    *   Matched entries are merged:
        *   `display` Array: Entries are matched based on `locale`.
            *   Matched entries: The entry in the extending metadata overwrites the entry in the parent metadata.
            *   Unmatched entries from both arrays are included in the result.
    *   Unmatched entries from both `claims` arrays are included in the result.
*   **`ui_labels` Object**:
    *   Keys (UI element identifiers) are merged.
    *   For a common key, the arrays of localised strings are merged based on `locale`.
        *   Matched entries: The entry in the extending metadata overwrites the entry in the parent metadata.
        *   Unmatched entries from both arrays are included in the result.
            ...
            "claims": [
                {
                    "path": ["payload", "amount"],
                    "display": [
                        { "locale": "de", "label": "Betrag" }
                    ]
                },
                {
                    "path": ["payload", "currency"],
                    "display": [
                        { "locale": "en", "label": "Currency" }
                    ]
                }
            ],
            "ui_labels": {
...
This specification extends the `transaction_data` object defined in [OID4VP] by adding a `payload` and `subtype` parameter. This parameter is an object with the transaction details, and its structure is defined by the [JSON Schema] referenced in the `type` parameter of the SCA Attestation type metadata.

- **`type`**: **REQUIRED** As specified in [OID4VP].
- **`subtype`**: **OPTIONAL** (string) A string that can be used to further categorize the transaction type.
- **`credential_ids`**: **REQUIRED** As specified in [OID4VP].
- **`transaction_data_hashes_alg`**: **REQUIRED** As specified in [OID4VP].
- **`payload`**: **REQUIRED** A JSON object containing the details for the transaction. The object is described by the [JSON Schema] referenced in the `type` parameter given by the attestation's type metadata.
...
- [`urn:eudi:sca:generic:1`](./api/ts12-urn-eudi-sca-generic-1-data-model.json)

#### 4.3.1 Payment Confirmation

The content of the `payload` object for the transaction data type `urn:eudi:sca:payment:1`. This transaction data type is meant to be used in conjunction with an SCA Attestation that represents an account or a card. The `subtype` parameter **MUST NOT** be present for this transaction type.
...
#### 4.3.2 Generic Transaction

The content of the `payload` object for the transaction data type `urn:eudi:sca:generic:1`. This transaction data type is meant to be used for various use cases, such as login, risk-based authentication, account information access, or e-mandates. The `subtype` parameter **MUST** be present for this transaction type.

- **`transaction_id`**: **REQUIRED** (string) Unique identifier of the Relying Party's interaction with the User.
- **`payment_payload`**: **OPTIONAL** (object) Object of the type `urn:eudi:sca:payment:1` as defined in Section 4.3.1. This parameter **MAY** be used to leverage that data structure, for instance to obtain a mandate for MITs.
- **Free fields**: The object **MAY** contain up to 10 additional custom text fields to provide additional details to the context of the transaction. The keys for these fields are not fixed but **MUST** be strings. The values **MUST** be strings or `null`. The total number of properties in the payload object is limited to 11. If a value is `null`, the key **MUST** be displayed without an associated value.
- **Formatting**: Generic fields **MUST** be displayed as a user-friendly date if they are an ISO date string, **MUST** be displayed as a user-friendly time if they are an ISO time string, and **MUST** be displayed as a user-friendly date-time if the value is an ISO date-time string. A Wallet Provider **MAY** support other ISO displays, like currency, country, language, etc.
- **Ordering**: Wallet Units **MUST** ensure that fields are displayed in the order they appear in the JSON object.
...
- Additionally, for Generic transactions as per Section 4.3.2:
  - a key-value pair where the key is "transaction_data_types.name" (string) and the value is populated with "Generic Transaction" (string),
